...
final class Asset_Loader implements Asset_Loader_Interface {
	...
	private function maybe_register_assets(): void {
		foreach ( self::$registry as $asset ) {
			...
			if ( 'css' === $asset['type'] ) {
				wp_register_style( $handle, $asset['src'], $asset['dependencies'] ?? [], $version );
			} else {
				wp_register_script( $handle, $asset['src'], $asset['dependencies'] ?? [], $version, true );
			}
	...
	private function collect_and_enqueue_assets( array $active_field_types, array $field_configs ): void {
		...
		$this->enqueue_code_editors( $field_configs );
